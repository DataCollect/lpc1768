                              ...
                              I2C_M_SETUP_Type *TransferCfg)
{
	...
	switch(CodeStatus)
	{
		case I2C_I2STAT_M_TX_START:   //0x08
		case I2C_I2STAT_M_TX_RESTART: //0x10
		//case I2C_I2STAT_M_RX_START:
		//case I2C_I2STAT_M_RX_RESTART
			// Send data first
//             if ((TransferCfg->send_opt_type == SEND_RD_ADR_OPT) || \
//                 (TransferCfg->send_opt_type == SEND_DATA_OPT))
            if (TransferCfg->send_opt_type == SEND_RD_ADR_OPT) 
            {
                /* Send slave address + WR direction bit = 0  */
				I2C_SendByte(I2Cx, (TransferCfg->sl_addr7bit & 0xfe));
				Ret = I2C_BYTE_SENT;    
            }
            else
            {
...
// 			}
			break;
		case I2C_I2STAT_M_TX_SLAW_ACK: //0x18
		case I2C_I2STAT_M_TX_DAT_ACK:  //0x28
			if (TransferCfg->send_opt_type == SEND_DATA_OPT)
            {
                if(TransferCfg->tx_count < TransferCfg->tx_length)
                {
            ...
            if (TransferCfg->send_opt_type == SEND_RD_ADR_OPT)
            {
                if (TransferCfg->reg_addr_byte_nbr == 2)
                {
                    I2C_SendByte(I2Cx, (TransferCfg->reg_addr >> 8) & 0xff); 
                    TransferCfg->reg_addr_byte_nbr--;
                    Ret = I2C_BYTE_SENT; 
                    //break;                    
                }
                else if (TransferCfg->reg_addr_byte_nbr == 1)
                {
                    I2C_SendByte(I2Cx, (TransferCfg->reg_addr & 0xff)); 
                    TransferCfg->reg_addr_byte_nbr--;
                    Ret = I2C_BYTE_SENT; 
                    //break;                    
                }
                else if (TransferCfg->reg_addr_byte_nbr == 0)
                {
                    //I2C_SendByte(I2Cx, (TransferCfg->reg_addr & 0xff)); 
                    //TransferCfg->reg_addr_nbr--;
                    I2Cx->I2CONCLR = I2C_I2CONCLR_SIC|I2C_I2CONCLR_STOC|I2C_I2CONCLR_STAC;
		            I2Cx->I2CONSET = I2C_I2CONSET_STA;
            ...
            if (TransferCfg->send_opt_type == SEND_WR_ADR_OPT)
            {
                if (TransferCfg->reg_addr_byte_nbr == 2)
                {
                    I2C_SendByte(I2Cx, (TransferCfg->reg_addr >> 8) & 0xff); 
			...
			break;
		case I2C_I2STAT_M_TX_DAT_NACK: //0x30
			I2C_Stop(I2Cx);
			Ret = I2C_SEND_END;
			break;
		case I2C_I2STAT_M_RX_ARB_LOST: //0x38
		//case I2C_I2STAT_M_TX_ARB_LOST:
			I2Cx->I2CONSET = I2C_I2CONSET_STA|I2C_I2CONSET_AA;
			I2Cx->I2CONCLR = I2C_I2CONCLR_SIC;
			break;
		case I2C_I2STAT_M_RX_SLAR_ACK: //0x40
			I2Cx->I2CONSET = I2C_I2CONSET_AA;
			I2Cx->I2CONCLR = I2C_I2CONCLR_SIC;

			Ret = I2C_BYTE_RECV;
			break;
		case I2C_I2STAT_M_RX_DAT_ACK:  //0x50
			...
			break;
		case I2C_I2STAT_M_RX_DAT_NACK:  //0x58
			I2C_GetByte(I2Cx, &tmp, FALSE);
			*rxdat++ = tmp;
			TransferCfg->rx_count++;
			I2C_Stop(I2Cx);
			Ret = I2C_RECV_END;
			break;
		case I2C_I2STAT_M_RX_SLAR_NACK:  //0x48
		case I2C_I2STAT_M_TX_SLAW_NACK:  //0x20
		case I2C_I2STAT_BUS_ERROR:       //0x00
			// Send STOP condition
			I2C_Stop(I2Cx);
			Ret = I2C_ERR;
			break;
...
//  * new data.
//  **********************************************************************/
// Bool I2C_MonitorHandler(LPC_I2C_TypeDef *I2Cx, uint8_t *buffer, uint32_t size)
// {
// 	Bool ret = FALSE;
